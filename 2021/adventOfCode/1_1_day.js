...
for (const command of input){
    // here's where we start using OOP
    // 
    if (command.substring(0, 1) == 'f') {
        horiz += parseInt(command.substring(8, command.length), 10);
    } else if (command.substring(0, 1) == 'd') {
        depth += parseInt(command.substring(5, command.length), 10);
    } else if (command.substring(0, 1) == 'u') {
        depth -= parseInt(command.substring(3, command.length), 10);
    } else {
        console.error('problem Day 2: Dive!');
        process.exit(1);
    };     depth: 1022
}

// console.log('horizontal: ' + horiz); // horizontal: 1996
// console.log('     depth: ' + depth); //      depth: 1022
// console.log('product: ' + horiz * depth); // product: 2039912
...
for (const command of input){ // took extra time debugging a copy of pt 1
    // console.log(`command: ${command}`);
    if (command.substring(0, 1) == 'f') {
        tempParseX = parseInt(command.substring(8, command.length), 10);
        horiz += tempParseX;
        depth += aim * tempParseX;
    } else if (command.substring(0, 1) == 'd') {
        aim += parseInt(command.substring(5, command.length), 10);
    } else if (command.substring(0, 1) == 'u') {
        aim -= parseInt(command.substring(3, command.length), 10);
    } else {
        console.error('problem Day 2:pt2 Dive!');
        process.exit(1);
    };
...
// perhaps there's value in doing binary operations, but...

// for (const diag in diagnosticReport) {
//     console.log(diag.toString());
// }
diagnosticReport.forEach(function(item, index, array) {
    console.log(item)
    item.forEach
  });
