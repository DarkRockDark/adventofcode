...
let prioritySum = 0;
const differenceFromA = -38;
const differenceFroma = -96;
...
function findUniqueCharInString(str) {
...
console.log("\n continuing ... 3.2");
let commonBadgeSum = 0;
let shortCircuit;

for (i = 0; i < rucksacks.length; i++) {
  if ((i + 1) % 3 == 0) {
    shortCircuit = false;
    // console.log(rucksacks[i-2]);
    for (const chr of rucksacks[i-2]) {
      // console.log(chr);
      if (
        !shortCircuit &&
        rucksacks[i-1].indexOf(chr) !== -1 &&
        rucksacks[i-0].indexOf(chr) !== -1
      ) {
        console.log("common found: " + chr);
        if (isUpperCase(chr)) {
          commonBadgeSum += chr.charCodeAt(0) + differenceFromA;
          shortCircuit = true;
        } else {
          commonBadgeSum += chr.charCodeAt(0) + differenceFroma;
          shortCircuit = true;
...
console.log("common Badges sum: " + commonBadgeSum);
